...
;(function($) {
	...
	$.datePicker = {
		strings: {
			...
			messageLocked: 'Выбранный день недоступен!'
		},
		defaults: {
		...
		show: function(options) {
			...
			var calendar = $.datePicker.utils.buildCalendar(selected.year, selected.month, date),
				...
				datePicker = $('<div class="datepicker"><span class="tip"><span> </span></span></div>');
			...
			if (opts.element) {
				...
				console.log('opts.element.outerHeight(true)', opts.element.outerHeight(true));
				console.log('opts.element.outerWidth(true)', opts.element.outerWidth(true));
				if (opts.hasOwnProperty('alignRight') && opts.alignRight) {
					datePicker.css({
						left: offset.left + datePicker.outerWidth(true) / 2 + 15 + 'px',
						top: offset.top - datePicker.outerHeight(true) / 2 + 15 + 'px'
					});
					$('.tip', datePicker).css({
						left: '-22px',
						top: datePicker.outerHeight(true) / 2 - 10 + 'px',
						'border-top': '10px solid transparent',
				    'border-bottom': '10px solid transparent',
				    'border-right': '12px solid #CCC'
					});
					$('.tip span', datePicker).css({
						left: '-6px',
    				top: '-8px',
			...
			// Calendar events
			datePicker.on('click.datePicker', '.calendar .day', function(e) {
				e.preventDefault();
				var el = $(this),
			...
			datePicker.on('click.datePicker', '.calendar .month', function(e) {
				e.preventDefault();
				var el = $(this),
					calendar = el.closest('.calendar'),
			...
			datePicker.on('click.datePicker', '.calendar .prev', function(e) {
				e.preventDefault();
				var el = $(this),
					calendar = el.closest('.calendar'),
			...
			datePicker.on('click.datePicker', '.calendar .next', function(e) {
				e.preventDefault();
				var el = $(this),
					calendar = el.closest('.calendar'),
					current = calendar.find('.month'),
			...
			if (opts.hasOwnProperty('show_months') && opts.show_months) {
				$('.calendar', datePicker).hide();
				$('.calendar .month', datePicker).trigger('click.datePicker');
			}

			// Month-picker events
			datePicker.on('click.datePicker', '.months .month', function(e) {
				e.preventDefault();
				var el = $(this),
				...
				// Change current selection
				selected.month = month;
				if (opts.hasOwnProperty('show_months')) {
					selected.day = 1;
					var test = new Date();
					test.setDate(selected.day);
					test.setMonth(selected.month);
			...
			datePicker.on('click.datePicker', '.months .prev', function(e) {
				e.preventDefault();
				var el = $(this),
					months = el.closest('.months'),
			...
			datePicker.on('click.datePicker', '.months .next', function(e) {
				e.preventDefault();
				var el = $(this),
					months = el.closest('.months'),
					current = months.find('.year'),
			...
			datePicker.on('click.datePicker', '.months .year', function(e) {
				e.preventDefault();
				var el = $(this),
					months = el.closest('.months'),
			...
			if (opts.hasOwnProperty('show_years') && opts.show_years) {
				$('.calendar', datePicker).hide();
				$('.month', datePicker).hide();
				$('.months .year', datePicker).trigger('click.datePicker');
			}

			// Year-picker events
			datePicker.on('click.datePicker', '.years .year', function(e) {
				e.preventDefault();
				var el = $(this),
				...
				if (opts.hasOwnProperty('show_years')) {
					selected.day = 1;
					selected.month = 1;
					var test = new Date();
					test.setDate(selected.day);
			...
			datePicker.on('click.datePicker', '.years .prev', function(e) {
				e.preventDefault();
				var el = $(this),
					years = el.closest('.years'),
			...
			datePicker.on('click.datePicker', '.years .next', function(e) {
				e.preventDefault();
				var el = $(this),
					years = el.closest('.years'),
					current = years.find('.decade'),
			...
			datePicker.on('click.datePicker', '.years .decade', function(e) {
				e.preventDefault();
				var el = $(this),
					years = el.closest('.years'),
			...
			datePicker.on('click.datePicker', '.decades .decade', function(e) {
				e.preventDefault();
				var el = $(this),
					decade = el.data('year'),
			...
			datePicker.on('click.datePicker', '.decades .prev', function(e) {
				e.preventDefault();
				var el = $(this),
					decades = el.closest('.decades'),
			...
			datePicker.on('click.datePicker', '.decades .next', function(e) {
				e.preventDefault();
				var el = $(this),
					decades = el.closest('.decades'),
					current = decades.find('.century'),
			...
			datePicker.on('click.datePicker', '.decades .century', function(e) {
				e.preventDefault();
				// Let's pick millenium! Bad idea, dropping it
			});
			$(document).on('mouseup.datePicker', function (e) {
				...
				{
					$(document).off('mouseup.datePicker');
					$.datePicker.hide();
				}
			});
		},
	...
	$.fn.datePicker = function(options) {
		...
		this.each(function() {
			...
			var callback = function(date) {
				...
				//opts.selectDate(date);
				return ret;
			};
			if (! button.length ) {
				// Bind to the element itself
				el.on('click.datePicker', function() {
					var oho_opts = $.extend(true, {element: el, selectDate: callback}, opts);
					$.datePicker.show(oho_opts);
				});
			} else {
